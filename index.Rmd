...
percent.change.table <- plot_ly(
  ...
  cells = list(
    values = rbind(round((((tail(crime.year$violent, 1) / crime.year$violent[1]) - 1)*100), 2),
                   round((((tail(crime.year$burglary, 1) / crime.year$burglary[1]) - 1)*100), 2),
                   round((((tail(crime.year$theft, 1) / crime.year$theft[1]) - 1)*100), 2),
                   round((((tail(crime.year$vehicle, 1) / crime.year$vehicle[1]) - 1)*100), 2)),
    align = c('center'),
    line = list(width = 1, color = 'black')
  ))
```
