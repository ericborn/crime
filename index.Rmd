...
yearly.change.pct.table <- plot_ly(
  ...
  cells = list(
    values = rbind(change.by.year.pct$measure, round(change.by.year.pct$Min,2), round(change.by.year.pct$X1st.Qu.,2), 
                   round(change.by.year.pct$Median,2),round(change.by.year.pct$Mean,2), 
                   round(change.by.year.pct$X3rd.Qu.,2), round(change.by.year.pct$Max,2)),
    align = c('center'),
    line = list(width = 1, color = 'black')
  ))
...
Studying the changes by year shows that the mean value of each category is on the decline as we previously saw. Vehicle theft and theft had the most significant reduction from 2008 to 2018, which is reconfirmed with their minimum values being close to four times less for burglary, -3000 to 771 and five times as much for vehicle theft -5962 to 1096.
